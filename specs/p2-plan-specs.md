...
| 领域 | 选型 | 说明 |
|------|------|------|
| 框架 | React 19 + Vite | 轻量 SPA，TypeScript 类型安全 |
| UI 组件 | Radix UI + Tailwind CSS | 无头组件库 + 原子化 CSS，快速构建 |
| 状态管理 | Zustand | 轻量、无 Redux 样板 |
| HTTP 客户端 | TanStack Query | 数据获取 + 缓存管理 |
| 实时通信 | Server-Sent Events (SSE) | 面试对话流式输出，轻量单向推送 |

#### 9.7.2 前后端通信方式

| 通道 | 协议 | 用途 |
|------|------|------|
| 面试控制 | REST API (FastAPI) | 启动面试、提交回答、人工仲裁操作 |
| 实时对话 | SSE (流式) | 提问官生成的问题逐字输出、评估进度推送 |
| 文件上传 | REST API (multipart) | 简历 PDF 上传 |
| 状态同步 | REST API (轮询) | 面试进度查询、历史面试回放 |

#### 9.7.3 前端状态管理

前端维护与后端 LangGraph 状态的对齐映射：

| 后端状态 | 前端状态 | 同步方式 |
|----------|----------|----------|
| `InterviewState` 整体 | `interviewStore` (Zustand) | 每次 API 响应后完整替换 |
| `chat_history` | 对话消息列表 | SSE 逐条追加 |
| `routing_flag` | UI 页面路由 | `CONTINUE` → 问答页；`ESCALATED` → 仲裁面板；`COMPLETED` → 报告页 |
| `evaluation_records` | 评分可视化组件 | REST 轮询 / 最终报告一次性拉取 |
| `interview_plan` | 议题进度条 | REST 拉取，高亮当前议题 |

#### 9.7.4 前端项目目录

采用 **monorepo 结构**，后端 `src/` 与前端 `web/` 同级：
...
│   │   ├── stores/         # Zustand 状态管理
│   │   ├── services/       # API 客户端（REST + SSE）
│   │   ├── types/          # TypeScript 类型定义（与后端 Pydantic 模型对齐）
│   │   └── App.tsx
│   ├── index.html
│   ├── vite.config.ts
│   └── package.json
├── tests/                  # 后端测试
├── tests-e2e/              # 端到端测试（Playwright，覆盖前后端联调）
...
| 策略 | 实现方式 |
|------|----------|
| 类型生成 | 使用 `datamodel-code-generator` 从后端 Pydantic 模型自动生成 TypeScript 接口 |
| 存放位置 | `web/src/types/generated.ts`，前端不手动复制类型定义 |
| 同步时机 | 每次后端 `InterviewState` 变更后，CI 流水线自动重新生成 TS 类型 |